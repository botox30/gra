...
if not token:
    ...
    print("Please add your Discord bot token to the Secrets tool.")
    exit(1)

print("Bot token found, starting Discord bot...")

bot = lightbulb.BotApp(token=token,
                       intents=hikari.Intents.ALL_UNPRIVILEGED
                       | hikari.Intents.GUILD_MEMBERS
                       | hikari.Intents.MESSAGE_CONTENT,
                       default_enabled_guilds=[1395873913514229881],
                       prefix="!")
...
async def on_start(event: hikari.StartedEvent):
    print(f"Bot is starting up... User: {bot.get_me()}")
    print(f"Bot is in {len(bot.cache.get_guilds_view())} guilds")
    ...
    role_manager.init_role_manager(bot)
    ...
    print("Bot started successfully and webserver is running on port 6000")
...
async def on_error(event: lightbulb.CommandErrorEvent) -> None:
    ...
    print(f"Command error: {type(exception).__name__}: {exception}")

config = utils.get_config()

async def access_notif(state, user_id, guild_data):
    member = await bot.rest.fetch_member(guild=guild_data.get("guild_id"),
                                         user=user_id)
    ...
    if state == "added":
        embed = hikari.Embed(
            ...
            description=f"Thank you for choosing Proofly {member.mention}"
            f"\nyou can now use the receipt generator by typing\n /menu or /generator",
            color=config["color"]).add_field(
                "Tutorial",
                f"We recommend you watch {generator_tutorial}").add_field(
                    "Vouch",
                    f"Please vouch for us in <#{config['vouch_channel']}>")

    elif state == "emulator":
        embed = hikari.Embed(
            title="Emulator Access Added",
            description=f"Thank you for choosing Proofly {member.mention}"
            f"\nyou can now use the emulators by going to\n https://proofly.net/account",
            color=config["color"]).add_field(
                "Tutorial",
                f"We recommend you watch {paper_generator_tutorial}"
            ).add_field(
                "Vouch",
                f"Please vouch for us in <#{config['vouch_channel']}>")
        ...
        embed = hikari.Embed(
            ...
            f"https://proofly.net/account\n"
            f"for email receipt generator type /menu or /generate",
            color=config["color"]).add_field(
                "Tutorial",
                f"We recommend you watch {paper_generator_tutorial},"
                f" {generator_tutorial} and {emulator_tutorial}").add_field(
                    "Vouch",
                    f"Please vouch for us in <#{config['vouch_channel']}>")
        ...
        embed = hikari.Embed(
            ...
            description=
            f"{member.mention}, your subscription has ended, and access to the receipt generator has been "
            f"removed",
            color="#ff244c",
    ...
    await bot.rest.create_message(embed=embed,
                                  channel=notification_channel,
                                  user_mentions=True)
    mention = await bot.rest.create_message(content=member.mention,
                                            channel=notification_channel,
                                            user_mentions=True)
...
async def add_access_role(request):
    ...
    if data.get("type") in ["emulator", "full_package", "paper_receipts"]:
        asyncio.create_task(
            access_notif(data["type"], int(data["user_id"]), guild_data))
    ...
    asyncio.create_task(access_notif("added", int(data["user_id"]),
                                     guild_data))
...
async def honeypot(event: hikari.MessageCreateEvent):
    ...
    if event.message.channel_id == config["honeypot_channel"]:
        ...
        try:
            await bot.rest.ban_user(guild=event.message.guild_id,
                                    user=event.author,
    ...
    for guild_member in await guild_db.members_without_access():
        try:
            ...
            if guild_data.get("access_role"):
                ...
                await access_notif(state="removed",
...
async def ticket_channel_deleted(event: hikari.GuildChannelDeleteEvent):
    if event.channel.category_id == config["ticketsystem"]["category"]:
        await database.Ticket.delete_ticket(event.channel_id)

@bot.command
@lightbulb.command("ping", "Check if the bot is responding")
@lightbulb.implements(lightbulb.SlashCommand)
async def ping(ctx: lightbulb.Context):
    await ctx.respond("🏓 Pong! Bot is online and working!")

print("Loading extensions...")
bot.load_extensions_from("./cogs_rent/")
print("Loaded cogs_rent extensions")
bot.load_extensions_from("./cogs_dev/")
print("Loaded cogs_dev extensions")
bot.load_extensions_from("./cogs_shared/")
print("Loaded cogs_shared extensions")
