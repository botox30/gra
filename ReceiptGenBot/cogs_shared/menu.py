...
async def send_email_and_update_menu(ctx, brand, product, rmenu: menu.Menu):
    ...
    embed = hikari.Embed(
        ...
        .set_footer(text="Proofly by social")
...
async def service_disabled_embed():
    embed = hikari.Embed(
        ...
        description="It looks like your bot subscription has not been paid."
                    ...
                    "**if you think that this was a mistake please contact me here contact@proofly.net**"
    )
    return embed
...
class ReceiptStepOne(menu.Screen):
    ...
    async def slm_brand(self, ctx: miru.ViewContext, select: miru.TextSelect) -> None:
        ...
        # Access is already verified by role check in the command
        # Just need to check if email is set up
...
class BaseMenu(menu.Menu):
    ...
    async def on_timeout(self) -> None:
        try:
            ...
            print(f"Error in BaseMenu.on_timeout: {e}")
            import traceback
            traceback.print_exc()
...
async def generate(ctx: lightbulb.Context):
    # Check if user has the required role - this is the primary access check
    required_role_id = 1397929996340953170
    member_roles = [role.id for role in ctx.member.get_roles()]

    if required_role_id not in member_roles:
        await ctx.respond("❌ **No Active Subscription**\n\nYou don't have an active subscription to use this command. Please purchase access to continue.", flags=hikari.MessageFlag.EPHEMERAL)
        return

    # If user has the role, they have access - no need for additional database checks
