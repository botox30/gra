...
# tasks.load(bot)  # Not needed in newer versions
...
async def on_start(event: hikari.StartedEvent):
    ...
    asyncio.create_task(periodic_status_change())
    asyncio.create_task(periodic_access_check())

    await runner.setup()
    webserver = web.TCPSite(runner, host='0.0.0.0', port=6000)
    await webserver.start()

async def periodic_status_change():
    while True:
