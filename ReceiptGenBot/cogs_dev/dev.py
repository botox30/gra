...
@plugin.command
@lightbulb.decorators.app_command_permissions(hikari.Permissions.ADMINISTRATOR,
                                              dm_enabled=False)
@lightbulb.command(name="create_purchase_view",
                   description="Creates a Persistent purchase view")
@lightbulb.implements(lightbulb.SlashCommand)
async def make_purchase_embed(ctx: lightbulb.Context):
    view = ticketsystem.MainTicketView(timeout=None)

    await ctx.bot.rest.create_message(channel=ctx.channel_id,
                                      components=view,
                                      embed=view.embed)
    ctx.app.d.miru.start_view(view)
    await ctx.respond("Created successfully",
...
async def image_link(event: hikari.MessageCreateEvent):
    if event.channel_id == config["images_channel"] and not event.is_bot:
        ...
        await event.message.respond(content="Here is your direct image link\n"
                                    f"```{image_url}```",
                                    reply=True,
                                    mentions_reply=True)
...
@lightbulb.decorators.app_command_permissions(hikari.Permissions.ADMINISTRATOR,
                                              dm_enabled=False)
@lightbulb.command(name="make_honeypot",
                   description="Creates a Honeypot Message")
@lightbulb.implements(lightbulb.SlashCommand)
async def make_honeypot(ctx: lightbulb.Context):
    emoji = hikari.CustomEmoji(id=config["emojis"]["honeypot"],
                               name="honeypot",
                               is_animated=False)
    embed = hikari.Embed(
...
async def backup_server_msg(ctx: lightbulb.Context):
    embed = hikari.Embed(
        ...
        description=
        "Link your discord account so in the future you get joined in the backup server",
        color=config["color"])

    view = miru.View().add_item(
        miru.LinkButton(url="oauth url", label="Link Account"))
    await ctx.bot.rest.create_message(channel=ctx.channel_id,
                                      embed=embed,
                                      components=view)
    await ctx.respond("Created successfully",
...
@lightbulb.decorators.app_command_permissions(hikari.Permissions.ADMINISTRATOR,
                                              dm_enabled=False)
@lightbulb.command(name="make_examples",
                   description="Creates Example Receipts in an Embed")
@lightbulb.implements(lightbulb.SlashCommand)
...
async def make_rules(ctx: lightbulb.Context):
    embed = hikari.Embed(
        title="Rules",
        description=
        "**These rules appy to you even if you don't care to read them, so do read them**\n"
...
async def make_info(ctx: lightbulb.Context):
    embed = hikari.Embed(
        title="Info",
        description="Basic information about Proofly",
        colour=config["color"]
    ).add_field(
        "How To Purchase?",
        "If you wish to purchase make a purchase ticket in <#1398026854937657344> more info in the channel"
    ).add_field(
        "Partnership?",
        "Sure if you provide something that could mutually benefit DM at <@750767957289992322>"
    ).add_field(
        "Vouch?",
        ...
        "making tiktok videos is recommended").set_thumbnail(
...
async def make_tos(ctx: lightbulb.Context):
    embed = hikari.Embed(
        ...
        value=
        "By using our service, you agree to comply with and be bound by these Terms of Service. If you do not "
        "agree with any part of these terms, please do not use our service. "
    ).add_field(
        name="2. No Liability",
        value=
        "We are not liable for any damages, losses, or issues that may occur as a result of using our service. "
        ...
        value=
        "We reserve the right to modify these Terms of Service at any time. Continued use of our service "
        "following any changes constitutes acceptance of the new terms. "
    ).add_field(
        name="5. Termination of Access",
        value=
        "We reserve the right to terminate your access to this service at any time and for any reason. This "
        ...
        value=
        "For any questions or to request a refund, please DM social with the necessary details."
    ).set_footer("TOS updated on 24.7 2025")
...
async def promote(ctx: lightbulb.Context):
    ...
    blacklisted = [
        1267128976631926785, 1277301275901300780, 1259292241915150387,
        1269322998909898852
    ]

    await ctx.bot.rest.create_message(channel=ctx.channel_id,
                                      content=f"found {len(guilds)} guilds")
    for guild in guilds:

        await asyncio.sleep(0.5)
        guild_members = len(ctx.bot.cache.get_members_view_for_guild(guild.id))
        await ctx.bot.rest.create_message(channel=ctx.channel_id,
                                          content=f"{guild_members} members")
