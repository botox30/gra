...
class Brand:
    ...
    async def send_email(to_email, html_content, subject, sender_name, spoofed_email=None):
        # You need to set these environment variables in the Secrets tab
        ...
        if not smtp_server or not smtp_user or not smtp_password:
            raise utils.GenerationError("email_config")
        ...
        try:
            ...
            raise utils.GenerationError("email_auth")
        except aiosmtplib.SMTPConnectError as e:
            print(f"SMTP Connection Error: {e}")
            print(f"SMTP Server: {smtp_server}, Port: {smtp_port}")
            raise utils.GenerationError("email_connect")
