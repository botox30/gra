...
load_dotenv('/root/amethyxbot/.env')
...
payment_options = {
    "Card": "💳",
    "PayPal": f"<:paypal_icon:{config['emojis']['paypal_icon']}>",
    "Other": f"<:stripe_icon:{config['emojis']['stripe_icon']}>",
    "GooglePay": f"<:google_pay_icon:{config['emojis']['google_pay_icon']}>",
    "ApplePay": f"<:applepay_icon:{config['emojis']['applepay_icon']}>",
    "MobilePay": f"<:mobile_pay_icon:{config['emojis']['mobile_pay_icon']}>",
    "Crypto": f"<:link_icon:{config['emojis']['link_icon']}>",
    "Blik": f"<:blik_icon:{config['emojis']['blik_icon']}>"
}
...
class TicketChannelView(miru.View):
    ...
    @miru.button(label="Close Ticket",
                 style=hikari.ButtonStyle.DANGER,
                 custom_id=f"{random.randint(1000, 99999)}")
    async def close(self, ctx: miru.ViewContext, button: miru.Button):
        close_confirm = hikari.Embed(
            title="Do you wish to close your ticket?",
            timestamp=datetime.datetime.now().astimezone())

        close_view = CloseConfirm()
        await ctx.respond(components=close_view,
                          embed=close_confirm,
...
class SetupTicket:
    ...
    def __init__(self, payment_option, subscription, bot, client, user_id,
                 guild):
    ...
    async def setup_ticket_channel(self, channel):
        subscription = config["ticketsystem"]["subscriptions"][
            self.subscription]
        ...
        try:
            ...
            print(f"DEBUG: Stripe payment link creation failed: {e}")
            print(f"DEBUG: Subscription: {self.subscription}")
            print(f"DEBUG: Payment option: {self.payment_option}")
            await channel.send(
                content=
                "Something went wrong while creating your payment link. Please contact support.",
        ...
        embed = hikari.Embed(
            ...
            color=config["color"],
            timestamp=datetime.datetime.now().astimezone()).add_field(
                "Amount", f"{subscription['price']}€")

        ticket_channel_view = TicketChannelView().add_item(
            miru.LinkButton(url=url, label="Payment Link"))
    ...
    def create_stripe_payment_link(self):
        ...
        stripe_price = config["ticketsystem"]["subscriptions"][
            self.subscription]["stripe_price"]
        currency = "EUR"
        ...
        elif self.payment_option == "Crypto":
        ...
        payment_link = stripe.PaymentLink.create(
            ...
            custom_text={
                "after_submit": {
                    "message": "Proofly will grant you access shortly"
                }
            },
            ...
            inactive_message=
            "Payment link is no longer active please create another ticket",
            after_completion={
                "type": "redirect",
                "redirect": {
                    "url":
                    f"https://discord.com/channels/{self.guild}/{config['notification_channel']}"
                }
            },
    ...
    async def create_ticket(self, member):
        print(f"DEBUG: Starting ticket creation for user {member.id}")
        ...
        print(f"DEBUG: Category ID: {category}")

        # Clean up any orphaned tickets for this user first
        print(f"DEBUG: Cleaning up old tickets for user {member.id}")
        cleanup_result = await database.Ticket.cleanup_user_tickets(member.id)
        print(f"DEBUG: Cleanup result: {cleanup_result}")

        try:
            print(
                f"DEBUG: Creating channel in guild {self.guild}, category {category}"
            )
            # Create channel with username format
            username = member.username if hasattr(member, 'username') else str(
                member.id)
            channel_name = f"ticket-{username}".lower().replace(
                ' ', '-')[:100]  # Discord channel name limit
            ...
            print(
                f"DEBUG: Channel created successfully: {channel.id} with name: {channel_name}"
            )
        except Exception as e:
            print(f"DEBUG: Failed to create channel: {e}")
            return None

        print(
            f"DEBUG: Calling database.Ticket.create_ticket with channel_id={channel.id}, user_id={member.id}"
        )
        result = await database.Ticket.create_ticket(channel_id=channel.id,
                                                     user_id=member.id)
        print(f"DEBUG: Database create_ticket result: {result}")

        if result is None:
            print(
                f"DEBUG: Ticket creation failed, deleting channel {channel.id}"
            )
            await channel.delete()
            return None

        print(f"DEBUG: Ticket created successfully with ID: {result}")
        # No need to rename since we already created it with the correct name
...
class PaymentOptionBtn(menu.ScreenButton):
    ...
    async def callback(self, ctx: miru.ViewContext):
        ticket = SetupTicket(bot=ctx.client.bot,
                             client=ctx.client,
        ...
        if channel_url:
            ...
            row = self.menu.client.app.rest.build_message_action_row(
            ).add_link_button(channel_url, label="Ticket")
            ...
            embed = hikari.Embed(
                ...
                color="#9966cc",
                description=
                "You may have exceeded the ticket limit, try to close your old one",
                timestamp=datetime.datetime.now().astimezone())
...
class PaymentOptionScreen(menu.Screen):
    ...
    async def build_content(self) -> menu.ScreenContent:
        return menu.ScreenContent(
            embed=hikari.Embed(title="Purchase Menu",
                               color="#9966cc",
    ...
    async def callback(self, ctx: miru.ViewContext):
        ...
        for payment_name, emoji in payment_options.items():
            payment_options_screen.add_item(
                PaymentOptionBtn(label=payment_name,
                                 emoji=emoji,
...
class BuyMenu(menu.Screen):
    ...
    async def build_content(self) -> menu.ScreenContent:
        return menu.ScreenContent(embed=hikari.Embed(
            title="Purchase Menu",
            color="#9966cc",
...
class MainTicketView(miru.View):
    ...
    brands_field = ""
    arrow_emoji = f"<:arrow_icon:{config['emojis']['arrow_icon']}>"
    ...
    embed = hikari.Embed(
        ...
        description=
        "This menu will take you to a private ticket\n where you can continue the payment process",
        color="#9966cc"
    ).add_field(
        f"{arrow_emoji} Email Receipt Generator", "- Day **5€**\n"
        "- Week **15€**\n"
        "- Forever **20€**\n"
        f"{brands_field}\n‎ "
    ).add_field("Payment Options", f"{payment_options_field}\n‎ ").add_field(
        "How does it work?",
        "Proofly will create a **stripe payment link** depending on selected payment "
    ...
    async def proceed_btn(self, ctx: miru.ViewContext, button: miru.Button):
        ...
        for subscription, conf in config["ticketsystem"][
                "subscriptions"].items():
            buy_menu.add_item(SubscriptionOptionBtn(label=subscription))

        builder = await my_menu.build_response_async(client,
                                                     buy_menu,
                                                     ephemeral=True)
