...
class UserData:
    base_url = "http://0.0.0.0:8000/api/discord-user/"
    ...
    async def get_expired_access_users() -> list:
        async with aiohttp.ClientSession() as session:
            async with session.get("http://0.0.0.0:8000/api/expired-access-users/") as response:
...
class Ticket:
    base_url = "http://0.0.0.0:8000/api/ticket/"
    ...
    @staticmethod
    async def cleanup_user_tickets(user_id):
        data = {
            "user_id": str(user_id)
        }
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{Ticket.base_url}cleanup-user/", json=data) as response:
                result = await response.json()
                return result.get("success", False)
