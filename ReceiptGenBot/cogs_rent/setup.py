...
async def setup_help(ctx: lightbulb.Context):
    ...
    embed = hikari.Embed(
        ...
        description=
        "Before you can start using this bot it needs to be activated as\n"
        "this is a paid service, **contact me here:**\n"
        "- Discord server https://amethyx.net/invite\n"
        "- Email 𝗰𝗼𝗻𝘁𝗮𝗰𝘁@proofly.net\n"
        ...
        "/remove_access - Admin Command", "Used for removing access from users"
    ).add_field(
        ...
        "you can login here: https://api.proofly.net/admin").add_field(
            "/access - User Command", "Shows the users access information"
        ).add_field(
...
@lightbulb.decorators.app_command_permissions(hikari.Permissions.ADMINISTRATOR,
                                              dm_enabled=False)
@lightbulb.command(name="manager",
                   description="If ran the first time crates an admin user")
@lightbulb.implements(lightbulb.SlashCommand)
async def manager(ctx: lightbulb.Context):

    url = "http://0.0.0.0:8000/api/guild/add-owner/"
    ...
    if not response.get("error", False):
        await ctx.respond(
            ...
            f"Login in to https://api.proofly.net/admin\n"
        ...
        await ctx.respond(
            ...
            "you can login at https://api.proofly.net/admin",
            flags=hikari.MessageFlag.EPHEMERAL)
...
@lightbulb.decorators.app_command_permissions(hikari.Permissions.ADMINISTRATOR,
                                              dm_enabled=False)
@lightbulb.option(name="role",
                  description="Choose a role",
                  required=True,
                  type=hikari.Role)
@lightbulb.command(
    name="setup_role",
    description="Setup access role that will be used by the bot")
@lightbulb.implements(lightbulb.SlashCommand)
async def setup_role(ctx: lightbulb.Context):
    ...
    if not result.get("error", False):
        await ctx.respond(f"{role.mention} will be used to track access",
                          flags=hikari.MessageFlag.EPHEMERAL)
...
@lightbulb.option(
    ...
    choices=["notifications", "purchase"],
    required=True,
)
@lightbulb.command(name="setup_channel",
                   description="Setup commands to choose channels for bot")
@lightbulb.implements(lightbulb.SlashCommand)
async def setup_channel(ctx: lightbulb.Context):
    ...
    if channel.type != hikari.ChannelType.GUILD_TEXT:
        await ctx.respond("Selected option is not a text channel",
                          flags=hikari.MessageFlag.EPHEMERAL)
        return
    ...
    if not result.get("error", False):
        await ctx.respond(
            f"{channel.mention} was selected as the {option} channel",
            flags=hikari.MessageFlag.EPHEMERAL)
